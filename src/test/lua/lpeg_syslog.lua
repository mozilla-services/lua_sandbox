...
local function traditional_file_format()
    ...
    assert(fields.timestamp == 1455027421000000000, fields.timestamp) -- update timestamp in 2017 unless https://github.com/mozilla-services/lua_sandbox/issues/108 is fixed
...
local function traditional_forward_format()
    ...
    local fields = grammar:match(log)
    assert(fields.msg == "imklog 5.8.6, log source = /proc/kmsg started.", fields.msg)
    assert(fields.timestamp == 1455093527000000000, fields.timestamp) -- update timestamp in 2017 unless https://github.com/mozilla-services/lua_sandbox/issues/108 is fixed
    assert(fields.syslogtag.programname == "kernel", fields.syslogtag.programname)
    assert(fields.pri.severity == 6, fields.pri.severity)
    assert(fields.pri.facility == 0, fields.pri.facility)
    assert(fields.hostname == "trink-x230", fields.hostname)
...
local function kitchen_sink()
    ...
    assert(fields.timegenerated == 1455096053000000000, fields.timegenerated) -- update timestamp in 2017 unless https://github.com/mozilla-services/lua_sandbox/issues/108 is fixed
