...
local kv_value = l.P'<' * l.C((l.P(1) - (l.P'>' * kv_pair_end))^1) * l.P'>'
                        + l.C((l.P(1) - kv_pair_end)^1)
...
local postfix_delays_cg = l.Ct(l.Cg(number, 'postfix_delay_before_qmgr')
                        * l.P'/'
...
local postfix_patterns = {
  ...
  -- lmtp = smtp,
  pickup = l.Ct(postfix_keyvalue_greedy_cg),
...
local postfix_programname = l.Ct(l.P'postfix'
                           * (l.P(1) - l.P'/')^0
                           * '/'
                           * l.Cg(l.P(1)^1, 'daemon'))
...
function M.postfix_match(programname, message, extract_keyvalue_data)
  local programtable = postfix_programname:match(programname)
  local ret = nil
  if postfix_patterns[programtable['daemon']] then
      ret = postfix_patterns[programtable['daemon']]:match(message)
  -- below are fake patterns used by tests
  elseif programtable['daemon'] == 'relay_info' then
    ret = l.Ct(postfix_relay_info_cg):match(message)
  elseif programtable['daemon'] == 'delays' then
    ret = postfix_delays_cg:match(message)
  end
  -- extract postfix_keyvalue_data
  if extract_keyvalue_data and ret['postfix_keyvalue_data'] then
    ...
    if kv then
      ...
      if ret['postfix_client'] then
        local cl = postfix_client_info_cg:match(ret['postfix_client'])
        if cl then
      ...
      if ret['postfix_relay'] then
        local relay = postfix_relay_info_cg:match(ret['postfix_relay'])
        if relay then
      ...
      if ret['postfix_delays'] then
        local delays = postfix_delays_cg:match(ret['postfix_delays'])
        if delays then
